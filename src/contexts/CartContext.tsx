...
export const CartProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const addToCart = async (item: Omit<CartItem, 'quantity'> & { quantity?: number }) => {
    ...
    try {
      ...
      // Calculate available stock based on selected variants
      let availableStock = 0;
      if (item.variants && item.variants.length > 0 && product.variants) {
        let minStock = Infinity;
        
        for (const selectedVariant of item.variants) {
          const productVariant = product.variants.find((v: { name: string }) => v.name === selectedVariant.variantName);
          if (!productVariant) continue;
          
          const option = productVariant.options.find((o: { value: string; label: string }) => 
          ...
          if (selectedVariant.subVariants && selectedVariant.subVariants.length > 0) {
            for (const subVariant of selectedVariant.subVariants) {
              const productSubVariant = option.subVariants?.find((sv: { name: string }) => sv.name === subVariant.subVariantName);
              if (!productSubVariant) continue;
              
              const subOption = productSubVariant.options.find((so: { value: string; label: string }) => 
      ...
      // Helper function to compare variants including sub-variants
      const compareVariants = (v1?: EnhancedVariant[], v2?: EnhancedVariant[]) => {
        ...
        return v1.every(variant => {
          const matchingVariant = v2.find(v => v.variantName === variant.variantName && v.optionValue === variant.optionValue);
          ...
          return sv1.every(subVariant =>
            sv2.some(sv => sv.subVariantName === subVariant.subVariantName && sv.optionValue === subVariant.optionValue)
          );
        });
      };
