...
const ProductPage = () => {
  ...
  // Handler functions for variants and add-ons
  const handleVariantChange = (variantName: string, optionValue: string) => {
    setSelectedVariants((prev) => {
      ...
      if (optionValue) {
        return [...filtered, { variantName, optionValue }];
      }
      return filtered;
    });
  };
          ...
          return (
            ...
            >
              ...
              <div>
                ...
                {/* Subcategories */}
                {Array.isArray((product as Product)?.subcategories) && (
                  <div className="mt-4">
                    <h4 className="font-semibold">Subcategories</h4>
                    <div className="flex flex-wrap gap-2 mt-1">
                      {(product as Product)?.subcategories?.map((sc: string, idx: number) => (
                        <span
                          key={`${sc}-${idx}`}
        ...
        <div className="mt-12">
          ...
          <div className="mt-6">
            {activeTab === 'description' && (
              <p className="text-gray-700">{fetchedproduct[0]?.description}</p>
            )}
            {activeTab === 'reviews' && (
